...
/// When writing a match expression against `GuardrailAction`, it is important
/// to ensure your code is forward-compatible. That is, if a match arm handles a
/// case for a feature that is supported by the service but has not been
/// represented as an enum variant in a current version of SDK, your code should
/// continue to work when you upgrade SDK to a future version in which the enum
/// does include a variant for that feature.
///
...
/// `GuardrailAction::NewFeature` in the current version of SDK. The reason is
...
/// calling `as_str` on `GuardrailAction::NewFeature` also yielding
/// `"NewFeature"`.
///
