...
/// When writing a match expression against `CachePointType`, it is important to
/// ensure your code is forward-compatible. That is, if a match arm handles a
...
/// calling `as_str` on `CachePointType::NewFeature` also yielding
/// `"NewFeature"`.
///
