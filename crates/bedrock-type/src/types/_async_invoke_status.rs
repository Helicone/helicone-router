...
/// When writing a match expression against `AsyncInvokeStatus`, it is important
...
/// `AsyncInvokeStatus::NewFeature` in the current version of SDK. The reason is
...
/// calling `as_str` on `AsyncInvokeStatus::NewFeature` also yielding
/// `"NewFeature"`.
///
