...
/// When writing a match expression against `GuardrailPiiEntityType`, it is
...
/// `GuardrailPiiEntityType::NewFeature` in the current version of SDK. The
/// reason is that the variable `other`, created by the `@` operator, is bound
/// to `GuardrailPiiEntityType::Unknown(UnknownVariantValue("NewFeature".
/// to_owned()))` and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `GuardrailPiiEntityType::NewFeature` is
/// defined. Specifically, when `guardrailpiientitytype` represents
/// `NewFeature`, the execution path will hit the second last match arm as
/// before by virtue of calling `as_str` on `GuardrailPiiEntityType::NewFeature`
