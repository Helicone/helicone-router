...
impl Mock {
    pub async fn new(config: &mut Config, args: MockArgs) -> Self {
        ...
        let openai_mock = start_mock("./stubs/openai", &args).await;
        config
            .providers
        ...
        let anthropic_mock = start_mock("./stubs/anthropic", &args).await;
        config
            .providers
        ...
        let minio_mock = start_mock("./stubs/minio", &args).await;
        config.minio.host = Url::parse(&minio_mock.uri()).unwrap();

        let jawn_mock = start_mock("./stubs/jawn", &args).await;
...
async fn start_mock(stub_path: &str, args: &MockArgs) -> Stubr {
    let active_stubs = args
        .stubs
        .as_ref()
        .map(|stubs| stubs.keys().copied().collect());
    ...
    if let Some(stub_expectations) = &args.stubs {
